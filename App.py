...
#     port=3306,
#     user='root',
#     password='MsIZSyNGkXRRBwrlYllmdlZIMFHGHmXs',
#     database='railway'
# )
...
#         if st.button('Login'):
#             if ad_user == 'dinesh' and ad_password == 'dinesh123':
#                 st.success("Welcome Mr. Admin !")
#                 # Display Data
...
#                 ## Pie chart for predicted field recommendations
#                 labels = plot_data["Predicted_Field"].unique()
#                 labels = [x.decode("utf-8") if isinstance(x, bytes) else x for x in labels]  # Convert bytes to strings
...
#                 # st.subheader("**Pie-Chart for User's Experienced Level**")
#                 # fig = px.pie(plot_data, values=values, names=labels, title="User's Experienced Level")
#                 # st.plotly_chart(fig)
